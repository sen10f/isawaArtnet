...
if __name__ == "__main__":
    ...
    # Synchronous Demo
    print("\n[Demo 1] Synchronous Controller (Unicast)")
    print("Sending DMX data to a specific device...")
    
    try:
        # IMPORTANT: Replace with your actual device IP
        TARGET_IP = "192.168.1.100"  # ← CHANGE THIS!
        
        with ArtNetController(TARGET_IP) as artnet:
            artnet.enable_debug(True)
            ...
            # Send to universe 0
            artnet.send_dmx(frame, universe=0)
            
            stats = artnet.get_stats()
            print(f"✓ Stats: {stats}")
    
    except ValueError as e:
        print(f"✗ Configuration error: {e}")
        print(f"  → Please specify a valid unicast IP address")
    except ArtNetError as e:
        print(f"✗ Network error: {e}")
    
    # Asynchronous Demo
    print("\n[Demo 2] Asynchronous Controller (Unicast)")
    print("Non-blocking DMX animation...")
    ...
    async def async_demo():
        try:
            ...
            async with AsyncArtNetController(TARGET_IP) as artnet:
                artnet.enable_debug(True)
                
                # Simulate smooth RGB animation
                for i in range(50):
                    phase = i * 0.2
                    
                    r = int(127 * (math.sin(phase) + 1))
                    g = int(127 * (math.sin(phase + 2) + 1))
                    b = int(127 * (math.sin(phase + 4) + 1))
                    
                    frame.set_channels(1, [r, g, b])
                    
                    # Non-blocking send
                    await artnet.send_dmx(frame, universe=0)
                    
                    # Other async tasks could run here
                    await asyncio.sleep(0.02)  # 50fps
                
                stats = artnet.get_stats()
                print(f"✓ Async animation completed: {stats['packets_sent']} packets")
    ...
    print("\nREMEMBER:")
    print("  • ALWAYS specify the target device IP address")
    print("  • ArtNetController      → Simple, synchronous")
    print("  • AsyncArtNetController → Modern, non-blocking")
    print("  • For GUI apps, ALWAYS use AsyncArtNetController!")
    print("="*70)
